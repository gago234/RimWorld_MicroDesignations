    ...
    public class WorkGiver_MicroRecipe : WorkGiver_Scanner
    {
        public override IEnumerable<Thing> PotentialWorkThingsGlobal(Pawn pawn)
        {
            ...
            foreach (Designation des in pawn.Map.designationManager.SpawnedDesignationsOfDef(mdef.designationDef))
            {
                ...
                {
                    Log.ErrorOnce("MicroRecipe designation has no target.", 63126, false);
                }
                else
                {
        ...
        public override Job JobOnThing(Pawn pawn, Thing t, bool forced = false)
        {
            ...
            foreach (var user in mdef.recipeDef.AllRecipeUsers)
            {
                List<Building> buildings = t.Map.listerBuildings.AllBuildingsColonistOfDef(user).Where(
                    x => (x as IBillGiver) != null && (x as IBillGiver).CurrentlyUsableForBills()
                    && pawn.CanReserve(x, 1, -1, null, forced) && pawn.CanReach(x, PathEndMode.ClosestTouch, Danger.Deadly, false, TraverseMode.ByPawn)).ToList();
            ...
            job = new Job(JobDefOf.DoBill, building);
